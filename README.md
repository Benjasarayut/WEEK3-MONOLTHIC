...
## 📚 Full Course Plan (Course Journey)

This repository is used as the course workspace for the ENGSE207 Task Board Journey — a semester-long project that evolves the same application through multiple architectural styles so students can compare, refactor, and learn practical trade-offs.

High-level timeline (13 weeks):

- Week 3: Monolithic Architecture — Everything in one place (Node.js + Express + SQLite + Vanilla JS)
- Week 4: Layered (3-tier) Architecture — Presentation / Business / Data separation
- Week 5: Client-Server (REST API) — Separate frontend and backend projects, API documentation
- Week 6: N-Tier (Docker) — Physical separation with multiple containers (Nginx, app instances, Redis)
- Week 7: Event-Driven — Message broker (RabbitMQ) and pub/sub patterns
- Week 8-9: Microservices — Decompose into independent services with API Gateway
- Week 10-13: Final Project — Teams apply learned architectures to a final deliverable
- Week 14-15: Presentation & Reflection

Each week contains a focused lab with objectives, scaffold code, and README notes. See the `weekX/README.md` files for a quick starter and checklist for each week.

---

## 🧑‍💻 Developer Quick Run

Follow these commands to prepare and run the application locally (tested on Linux / WSL):

```bash
# 1) Install dependencies
npm install

# 2) Initialize the SQLite database (creates `database/tasks.db` and sample data)
...
If port 3000 is already in use, stop the running process or change the port in `server.js`.

---

If you want the full weekly breakdown copied into the root README, check the `week3`..`week15` folders added alongside this file.
